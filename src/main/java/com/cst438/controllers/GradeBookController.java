...
public class GradeBookController {
   ...
   EnrollmentRepository enrollmentRepository;
		...
		if (!assignments.isEmpty()) {
	      ...
	      return result;
		   ...
		   for (Enrollment e: enroll) {
		      assignments = assignmentRepository.findByCourseId(e.getCourse().getCourse_id());
		      for (Assignment a: assignments) {
	            result.assignments.add(new AssignmentListDTO.AssignmentDTO(a.getId(), a.getCourse().getCourse_id(), a.getName(), a.getDueDate().toString() , a.getCourse().getTitle()));
	         }
         }
	...
	public GradebookDTO getGradebook(@PathVariable("id") Integer assignmentId, @AuthenticationPrincipal OAuth2User principal ) {
      ...
      if (course.isEmpty() && !enroll.isEmpty()) {
         ...
         for (Enrollment e : enroll) {
            if (e.getCourse().getCourse_id() == assignment.getCourse().getCourse_id()) {
               GradebookDTO.Grade grade = new GradebookDTO.Grade();
               en = e;
               grade.name = en.getStudentName();
               grade.email = en.getStudentEmail();
               ...
               if (ag != null) {
                  ...
                  grade.grade = "";
                  AssignmentGrade agNew = new AssignmentGrade(assignment, en);
         ...
         assignment = checkAssignment(assignmentId, email);
         gradebook.assignmentId = assignmentId;
         gradebook.assignmentName = assignment.getName();
   ...
   @Transactional
   public AssignmentListDTO.AssignmentDTO addAssignment (@RequestBody AssignmentListDTO.AssignmentDTO assignDTO, @PathVariable int course_id, @AuthenticationPrincipal OAuth2User principal ) {
